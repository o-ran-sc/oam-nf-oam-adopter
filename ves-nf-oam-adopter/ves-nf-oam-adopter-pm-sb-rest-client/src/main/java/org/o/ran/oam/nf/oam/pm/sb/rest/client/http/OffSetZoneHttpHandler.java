...
import org.o.ran.oam.nf.oam.pm.sb.rest.client.pojos.TimeZoneResponse;
...
public final class OffSetZoneHttpHandler {

    private static final Logger LOG = LoggerFactory.getLogger(OffSetZoneHttpHandler.class);
     ...
     */
    public static Single<ZoneId> readTimeZone(final DefaultHttpRestClient httpRestClient, final Adapter adapter) {
        LOG.debug("Read Time Zone from adapter {}", adapter.getHostIpAddress());
        return httpRestClient.get(adapter, Urls.OFFSET_URL.get()).map(response -> validateGet(response, adapter))
                       .map(ZoneId::of);
    }
    ...
    private static String validateGet(final SimpleHttpResponse response, final Adapter adapter) {
        ...
        return GSON.fromJson(output, TimeZoneResponse.class).getTimeZoneOffset();
    }
}
